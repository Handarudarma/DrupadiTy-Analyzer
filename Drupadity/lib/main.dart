...
void main() async {
  ...
  runApp(const SecurityAnalyzeApp());
}
...
Future<Uint8List?> extractAppIcon(Uint8List apkBytes, String fileName) async {
  try {
    ...
    debugPrint('Error extracting app icon: $e'); // Using Flutter's built-in logging
    return null;
  }
}
...
class _ResultPageState extends State<ResultPage> with SingleTickerProviderStateMixin {
  ...
  final List<Tab> myTabs = <Tab>[
    const Tab(text: 'Info'),
    const Tab(text: 'Signer'),
    const Tab(text: 'Perm'),
    const Tab(text: 'AMAPI'),
  ];
...
class SignerPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Color textColor = isDarkMode ? Colors.white : Colors.black;
    Color cardColor = isDarkMode ? Colors.grey[900]! : Colors.white;
    debugPrint('=== CERTIFICATE DATA ===');
    debugPrint('Certificate Analysis: ${analysisData['certificate_analysis']}');
    debugPrint('========================');
...
// Removed unused cardColor variable
    ...
    debugPrint('=== Permission Data ===');
    debugPrint('Permissions: ${analysisData['permissions']}');
